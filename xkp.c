// xkp: Logs X11 keypresses, globally.
        ...
        if (XGetEventData(disp, cookie)) {
            ...
            if (cookie->type == GenericEvent
                ...
                if (cookie->evtype == XI_RawKeyRelease
                    ...
                    if(!strcmp("less", str)) {
                        send_key = '<';
                    } else if(!strcmp("comma", str)) {
                        send_key = ',';
                    } else if(!strcmp("period", str)) {
                        send_key = '.';
                    } else if(!strcmp("slash", str)) {
                        send_key = '/';
                    }

                    if(cookie->evtype == XI_RawKeyPress) {
                        audio_key_play(send_key);
                    } else {
                        audio_key_stop(send_key);
                    }
                }
            }
